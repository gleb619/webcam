			...
			<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
				<thead>
					...
					<c:if test="${debug}">
						...
						<th ng-if="::checkVisibility('token')">
							<div ng-click="sort('token')" >
								<a class="sortable-column-name"><span><spring:message code="label.token" /></span></a>
								<i class="fa" ng-class="{ 'fa-caret-down': details.token, 'fa-caret-up': !details.token }"></i>
							</div>
						</th>
				...
				<tbody>
					...
					<tr ng-repeat="item in filteredUsers track by $index"
					...
					<c:if test="${debug}">
						...
						<td ng-if="::checkVisibility('token')"><label>{{ ::item.token }}</label></td>
