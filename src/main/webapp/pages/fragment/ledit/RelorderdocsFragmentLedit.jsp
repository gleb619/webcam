		...
		<div class="padding-left-right-15">
			<div class="col-md-6 col-lg-6 without-padding">
				<div class="has-feedback searchBar">
					<input type="text" id="searchKeywordRelOrderDocs" name="searchKeywordRelOrderDocs"
							ng-model="searchKeywordRelOrderDocs" class="form-control"
			...
			<div class="btn-group col-md-6 col-lg-6 without-padding display-block toolbar-panel">
				...
				<button type="button" 
					class="btn btn-default btn-delete padding-top-2-bottom-4 
						   margin-left-0 col-xs-6 col-sm-6 col-md-6"
					ng-click="deleteSubItem('relOrderDocses', 'RelOrderDocs', selectedItemrelOrderDocses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in filteredRelOrderDocs = ( item.relOrderDocses | filter:searchKeywordRelOrderDocs )"
							ng-class="{ 'success' : selectedItemrelOrderDocses.id == item.id }"
							ng-click="setSelected('selectedItemrelOrderDocses', item, $index)">
						<td ng-show="checkVisibility('docdocuments')"><label>{{ item.docdocuments.label }}</label></td>
						<td ng-show="checkVisibility('tmorder')"><label>{{ item.tmorder.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relOrderDocses', 'RelOrderDocs', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ filteredRelOrderDocs.length }}/{{ item.relOrderDocses.length }}
				</div>
			  </div>
		</div>
	</div>
