		...
		<div class="padding-left-right-15">
			...
			<div class="btn-group col-md-6 col-lg-6 without-padding display-block toolbar-panel">
				<button type="button" 
					...
					ng-click="createModalItemTransferRoute()">
				...
				<button type="button" 
					class="btn btn-default btn-edit padding-top-2-bottom-4 
						   margin-left-0 col-xs-4 col-sm-4 col-md-4"
					ng-click="editModalItemTransferRoute(selectedItemtransferRoutes.id)">
				...
				<button type="button" 
					class="btn btn-default btn-delete padding-top-2-bottom-4 
						   margin-left-0 col-xs-4 col-sm-4 col-md-4"
					ng-click="deleteSubItem('transferRoutes', 'TransferRoute', selectedItemtransferRoutes.id, itemIndex)">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in filteredTransferRoute = ( item.transferRoutes | filter:searchKeywordTransferRoute )"
							ng-class="{ 'success' : selectedItemtransferRoutes.id == item.id }"
							ng-dblclick="editModalItemTransferRoute(item.id)"
						...
						<td ng-show="checkVisibility('currcurrency')"><label>{{ item.currcurrency.label }}</label></td>
						...
						<td ng-show="checkVisibility('descr')"><label>{{ item.descr }}</label></td>
							<td>
								<a ng-click="editModalItemTransferRoute(item.id)" class="btn">
									<i class="glyphicon glyphicons-lg glyphicon-pencil"></i> 
								</a>
							</td>
							<td>
								<a ng-click="deleteSubItem('transferRoutes', 'TransferRoute', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ filteredTransferRoute.length }}/{{ item.transferRoutes.length }}
				</div>
			</div>
		</div>
	</div>
