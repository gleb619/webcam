		...
		<div class="padding-left-right-15">
			...
			<div class="btn-group col-md-6 col-lg-6 without-padding display-block toolbar-panel">
				<button type="button" 
					...
					ng-click="pickItemRelPurchaseItems()">
				...
				<button type="button" 
					class="btn btn-default btn-delete padding-top-2-bottom-4 
						   margin-left-0 col-xs-6 col-sm-6 col-md-6"
					ng-click="deleteSubItem('relPurchaseItemses', 'RelPurchaseItems', selectedItemrelPurchaseItemses.id, itemIndex)">
		...
		<div class="col-md-12">
			  <div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in filteredRelPurchaseItems = ( item.relPurchaseItemses | filter:searchKeywordRelPurchaseItems )"
							ng-class="{ 'success' : selectedItemrelPurchaseItemses.id == item.id }"
							ng-click="setSelected('selectedItemrelPurchaseItemses', item, $index)">
						<td ng-show="checkVisibility('appticket')"><label>{{ item.appticket.label }}</label></td>
						<td ng-show="checkVisibility('pricepurchase')"><label>{{ item.pricepurchase.label }}</label></td>
							<td>
								<a ng-click="deleteSubItem('relPurchaseItemses', 'RelPurchaseItems', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ filteredRelPurchaseItems.length }}/{{ item.relPurchaseItemses.length }}
				</div>
			  </div>
		</div>
	</div>
