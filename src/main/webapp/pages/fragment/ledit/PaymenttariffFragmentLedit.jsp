		...
		<div class="padding-left-right-15">
			<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
				...
				<button type="button" class="btn btn-default btn-edit"
					ng-click="editModalItemPaymentTariff(selectedItempaymentTarifves.id)">
					<i class="glyphicon glyphicon-pencil pull-left"></i>
					<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.edit" /></span>
				</button>
				<button type="button" class="btn btn-default btn-delete"
					ng-click="deleteSubItem('paymentTarifves', 'PaymentTariff', selectedItempaymentTarifves.id, itemIndex)">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
					...
					<tbody>
						<tr ng-repeat="item in item.paymentTarifves track by $index"
							ng-class="{ 'success' : selectedItempaymentTarifves.id == item.id }"
							ng-dblclick="editModalItemPaymentTariff(item.id)"
							ng-click="setSelected('selectedItempaymentTarifves', item, $index)">
						...
						<td ng-if="::checkVisibility('synopsys')"><label>{{ ::item.synopsys }}</label></td>
							...
							<td>
								<a ng-click="deleteSubItem('paymentTarifves', 'PaymentTariff', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.paymentTarifves.length }}
				</div>
			</div>
		</div>
	</div>
