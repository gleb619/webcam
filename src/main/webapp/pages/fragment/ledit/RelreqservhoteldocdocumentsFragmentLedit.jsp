	...
	<div class="row padding-left-right-15">
		<div class="btn-group without-padding display-block toolbar-panel margin-bottom-5">
			<button type="button" class="btn btn-default btn-create"
				ng-click="createModalItemRelReqServHotelDocDocuments()">
				<i class="fa fa-file-o pull-left"></i>
				<i class="fa fa-file pull-left"></i>
				<span>&nbsp;&nbsp;&nbsp;&nbsp;<spring:message code="default.create" /></span>
			</button>
			<button type="button" class="btn btn-default btn-edit"
				ng-click="uploadRelReqServHotelDocDocuments()">
			...
			<button type="button" class="btn btn-default btn-reload"
				ng-click="pickItemRelReqServHotelDocDocuments()">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered margin-bottom-5 padding-all-2">
					...
					<tbody>
						<tr ng-repeat="item in item.relReqServHotelDocDocumentses track by $index"
							ng-class="{ 'success' : selectedItemrelReqServHotelDocDocumentses.id == item.id }"
							ng-dblclick="editModalItemRelReqServHotelDocDocuments(item.id)"
							ng-click="setSelected('selectedItemrelReqServHotelDocDocumentses', item, $index)">
						...
						<td ng-if="::checkVisibility('reqservhotel')"><label>{{ ::item.reqservhotel.label }}</label></td>
							<td>
								<a ng-click="editModalItemRelReqServHotelDocDocuments(item.id)" class="btn">
									<i class="glyphicon glyphicons-lg glyphicon-pencil"></i> 
								</a>
							</td>
							<td>
								<a ng-click="deleteSubItem('relReqServHotelDocDocumentses', 'RelReqServHotelDocDocuments', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ item.relReqServHotelDocDocumentses.length }}
				</div>
			</div>
		</div>
	</div>
