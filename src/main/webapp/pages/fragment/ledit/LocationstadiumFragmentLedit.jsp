		...
		<div class="padding-left-right-15">
			...
			<div class="btn-group col-md-6 col-lg-6 without-padding display-block toolbar-panel">
				<button type="button" 
					...
					ng-click="createModalItemLocationStadium()">
				...
				<button type="button" 
					class="btn btn-default btn-edit padding-top-2-bottom-4 
						   margin-left-0 col-xs-4 col-sm-4 col-md-4"
					ng-click="editModalItemLocationStadium(selectedItemlocationStadiums.id)">
				...
				<button type="button" 
					class="btn btn-default btn-delete padding-top-2-bottom-4 
						   margin-left-0 col-xs-4 col-sm-4 col-md-4"
					ng-click="deleteSubItem('locationStadiums', 'LocationStadium', selectedItemlocationStadiums.id, itemIndex)">
		...
		<div class="col-md-12">
			<div class="table-responsive margin-bottom-5 table-body ledit-scroll">
				<table class="table table-hover table-striped table-bordered 
					...
					<tbody>
						<tr ng-repeat="item in filteredLocationStadium = ( item.locationStadiums | filter:searchKeywordLocationStadium )"
							ng-class="{ 'success' : selectedItemlocationStadiums.id == item.id }"
							ng-dblclick="editModalItemLocationStadium(item.id)"
						...
						<td ng-show="checkVisibility('maplocation')"><label>{{ item.maplocation }}</label></td>
							<td>
								<a ng-click="editModalItemLocationStadium(item.id)" class="btn">
									<i class="glyphicon glyphicons-lg glyphicon-pencil"></i> 
								</a>
							</td>
							<td>
								<a ng-click="deleteSubItem('locationStadiums', 'LocationStadium', item.id, $index)" class="btn">
				...
				<div>
					<spring:message code="default.total" />: {{ filteredLocationStadium.length }}/{{ item.locationStadiums.length }}
				</div>
			</div>
		</div>
	</div>
