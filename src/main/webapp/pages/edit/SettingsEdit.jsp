	...
	<div ng-if="!loading" class="edit-form-body">
		<div class="modal-header header">
			...
			<h4 class="modal-title">
				...
				<c:if test="${debug}">
					<span>{{ item.id }}</span>
					<a href="../data/settings/{{ ::item.id }}?initLists=true&initDicts=true" target="_blank" class="btn btn-default">
						JSON DATA
					</a>
				</c:if>
				<!--
			  ...
			  <tab> 
				...
				<div class="col-sm-8 col-md-6 col-xs-offset-0 col-sm-offset-2 col-md-offset-3 edit-block">
					...
					<div class="form-group margin-top-5" ng-if="::checkVisibility('name')">
						...
						<input type="text" ng-model="item.name" class="form-control" tabindex="2" />
					</div>

					<div class="form-group margin-top-5" ng-if="::checkVisibility('value')">
						<label><spring:message code="label.value" /></label>
						<input type="text" ng-model="item.value" class="form-control" tabindex="3" />
					</div>
				</div>
